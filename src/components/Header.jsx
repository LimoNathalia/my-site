...
function MobileNavigation(props) {
  return (
    <Popover {...props}>
      <Popover.Button className="group flex items-center rounded-full bg-white px-4 py-2 text-sm font-medium text-black shadow-lg shadow-white ring-1 ring-primary backdrop-blur">
        Menu
        <ChevronDownIcon className="ml-3 h-auto w-2 stroke-black group-hover:stroke-primary" />
      </Popover.Button>
      <Transition.Root>
        <Transition.Child
          as={Fragment}
        ...
        >
          <Popover.Panel
            focus
            className="fixed inset-x-4 top-8 z-50 origin-top rounded-3xl bg-white p-8 ring-1 ring-primary"
          >
            <div className="flex flex-row-reverse items-center justify-between">
              <Popover.Button aria-label="Close menu" className="-m-1 p-1">
                <CloseIcon className="h-6 w-6 text-primary" />
              </Popover.Button>
              <h2 className="text-sm font-medium text-black">Menu</h2>
            </div>
            <nav className="mt-6">
              <ul className="-my-2 divide-y divide-black text-base text-black">
...
function DesktopNavigation(props) {
  return (
    <nav {...props}>
      <ul className="shadow-zinc-white/5 flex rounded-full bg-white px-3 text-sm font-medium text-black shadow-lg ring-1 ring-primary backdrop-blur">
...
function AvatarContainer({ className, ...props }) {
  return (
    <div
      className={clsx(
        className,
        'h-10 w-10 rounded-full bg-white/90 p-0.5 shadow-lg shadow-white/5 ring-1 ring-primary/5 backdrop-blur'
      )}
      {...props}
    />
  );
  ...
  return (
    <Link href="/" aria-label="Home" className={clsx(className, 'pointer-events-auto')} {...props}>
      <Image
        ...
        className={clsx('rounded-full bg-white object-cover', large ? 'h-16 w-16' : 'h-9 w-9')}
        priority
      />
    </Link>
  );
